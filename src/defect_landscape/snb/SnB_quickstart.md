...
4. Prepare the bulk `POSCAR` and defect `POSCAR` for the defect you want to screen.
5. Prepare VASP relaxation inputs in one folder: `INCAR`, `KPOINTS`, `POTCAR`, and optionally `submit.sh`.
6. Check if the model file is supported by reading `config.yml`. (IT IS), and if it isnt, add its calculator object to `src/mlip_phonons/get_calc.py`.
7. Run:
...
The case folder is inferred from the defect POSCAR parent folder. In the command
above, `path/to/v_I_q0/POSCAR` gives the case folder `v_I_q0`. If you want to
force a different label, add `--case-name my_defect_case`.
...
```bash
mlip-snb check-dft --results-root resultsSNB
mlip-snb compare-dft --results-root resultsSNB
mlip-snb report --results-root resultsSNB
```

If `resultsSNB/` contains more than one case folder, add `--case-name v_I_q0`
to those commands.

The structure you want is identified by `dft_ground_selection_id` in:

```text
resultsSNB/v_I_q0/analysis/mlip_vs_dft.csv
```

and the corresponding final DFT-relaxed ground-state structure is:

```text
resultsSNB/v_I_q0/dft_validate/<dft_ground_selection_id>/CONTCAR
```
