...
and the two endpoint `POSCAR` files, then feeds an in-memory configuration to
the same NEB engine used by the YAML interface; intermediate coordinates are
generated by IDPP and no temporary `config.yml` is created. See the [NEB
workflow guide](docs/workflows/neb.md) for the supported layout and optional
MLIP comment directives.
